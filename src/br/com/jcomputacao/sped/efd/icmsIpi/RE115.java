package br.com.jcomputacao.sped.efd.icmsIpi;

import br.com.jcomputacao.aristoteles.field.*;
import br.com.jcomputacao.aristoteles.format.FormatFactory;
import br.com.jcomputacao.aristoteles.format.FormatType;
import br.com.jcomputacao.aristoteles.format.FormatWrapper;
import br.com.jcomputacao.aristoteles.line.LineArchetype;

/**
 * 05/02/2013 17:47:14
 * @author Vinicius Diana
 */
public class RE115 extends LineArchetype {

    /*
      * 1 -  Texto fixo contendo "E115"
      */ 
    public static final String REGISTRO = "REG";
    
    
     /*
      * 2 -  Código da informação adicional conforme tabela a ser definida pelas 
      *  SEFAZ, conforme tabela definida no item 5.2.
      */
    public static final String COD_INF_ADIC = "COD_INF_ADIC";
    
    
    /*
      * 3 - Valor referente à informação adicional
      */
    public static final String VL_INF_ADIC = "VL_INF_ADIC ";
    
    
    
    /*
      * 4 - Descrição complementar do ajuste
      */
    public static final String DESCR_COMPL_AJ = "DESCR_COMPL_AJ";
    
    
    public RE115(){
        setName("Registro E115 - informacoes adicionais da apuracao - valores declaratorios");
        setDelimiter("|");
        //1
        addFieldArchetype(REGISTRO, new FieldDefaultArchetype("E115"));
        //2
        addFieldArchetype(COD_INF_ADIC, new FieldStringFixedLengthArchetype(8));
        //3
        FormatWrapper fw = FormatFactory.getFormat(FormatType.DECIMAL);
        fw.setReplaceComa(false);
         FieldDecimalMaximumLengthArchetype f0 = new FieldDecimalMaximumLengthArchetype(15,2);
        f0.setFormat(fw);
        addFieldArchetype(VL_INF_ADIC, f0);
        //4
        addFieldArchetype(DESCR_COMPL_AJ, new FieldStringMaximumLengthArchetype(255));
    }
    
    
    
}
